...
Summary: Greengage PXF framework for external data access
License: %{license}
URL: https://github.com/greengage-db/pxf
Vendor: %{vendor}

Prefix: /usr/local/%{name}

# .so file makes sense only when installing on Greengage node, so inherit Greengage's dependencies implicitly
# Java server can be installed on a new node, only bash is needed for management scripts
Requires: bash

%description
PXF is an extensible framework that allows a distributed database like Greengage to query external data files,
