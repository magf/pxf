...
* Greengage extension -- written in C; when built, produces a `pxf.so` library and configuration files
...
respectively. PXF compiles against and generates a different package for every major Greengage version.

For example, `pxf-gp5-1.2.3-1.el7.x86_64.rpm` represents an RPM package of PXF version 1.2.3 intended to work with
Greengage 5 on Centos / Redhat 7 operating systems.
...
* the name of the RPM package is `pxf-gpX`, where X is the major Greengage version (e.g. `pxf-gp5`, `pxf-gp6`)
* to install a newer RPM package for the same Greengage major release, a user will have to upgrade the PXF RPM
...
* the PXF greengage extension is initially installed by RPM alongside the PXF server and is not initially active
* the PXF greengage extension is copied into Greengage install location during `pxf init` command issued by a user after the install
...
2. Install Greengage database
3. Run `source $GPHOME/greengage_path.sh` to configure your `PATH` to be able to find `pg_config` program
...
1. Build or download PXF RPM for the corresponding major version of Greengage. The following example will assume
   that PXF version `1.2.3` will be installed to work with with Greengage 5.
2. Decide which OS user will own the PXF installation. If PXF is installed alongside Greengage, the user that owns the PXF
installation should either be the same as the one owning the Greengage installation or have write privilleges to the
Greengage installation directory. This is necessary to be able to register the PXF Greengage extension with Greengage.
...
PXF for the same major Greengage version, the files and the runtime directories from the older version will be removed.
