...
# To build centos image - run the command: GPDB_IMAGE=cloud-hub.adsw.io/library/gpdb6_u22:latest bash build-images.sh
pushd ../..
docker build -t gpdb6_pxf_automation:it --build-arg "GPDB_IMAGE=${GPDB_IMAGE:-cloud-hub.adsw.io/library/gpdb6_u22:latest}" -f automation/arenadata/Dockerfile .
popd
