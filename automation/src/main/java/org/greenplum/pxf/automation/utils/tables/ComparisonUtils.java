    ...
    private static boolean checkColData(int colType, String dataColT1, String dataColT2, Reporter report) {
        ...
        try {
            ...
            if (colType == Types.BINARY) {
                ...
                if ((Arrays.equals(dataColT1.getBytes(), dataColT2.getBytes(StandardCharsets.UTF_8)))
                        || (Arrays.equals(dataColT1.getBytes(StandardCharsets.UTF_8), "?".getBytes()))
                        || (Arrays.equals("?".getBytes(), dataColT2.getBytes()))) {
                    return true;
                }
            }
    ...
    public static boolean comparePsqlDescribeHive(String psqlOutput,
        ...
        if ((isEquals && PSQL_DESCRIBE_HEADER_OFFSET + hiveTables.size()
                * PSQL_DESCRIBE_HEADER_TABLE_OFFSET + totalFieldsNum
                ...
                || hiveTables.isEmpty() && psqlLines.length != 0) {
