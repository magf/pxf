...
public class JdbcEncryptionTest extends BaseFeature {
    ...
    @Test(groups = {"features", "jdbc"}, description = "Check wrong encrypted password")
    public void checkWrongEncryptedPassword() throws Exception {
        copyAndModifyJdbcConfFile(WRONG_ENCRYPTED_PASSWORD);
        runSqlTest("features/jdbc/encryption/error-encrypted");
    }

    @Test(groups = {"features", "jdbc"}, description = "Check encrypted password")
    public void checkEncryptedPassword() throws Exception {
        copyAndModifyJdbcConfFile(ENCRYPTED_PASSWORD);
        runSqlTest("features/jdbc/encryption/success-encrypted");
    }

    @Test(groups = {"features", "jdbc"}, description = "Check not encrypted password")
