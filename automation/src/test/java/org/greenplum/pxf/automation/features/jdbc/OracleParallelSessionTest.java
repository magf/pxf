...
public class OracleParallelSessionTest extends BaseFeature {
    ...
    @Test(groups = {"features", "jdbc"}, description = "Set default parameters for parallel queries")
    public void checkDefaultParamsForParallel() throws Exception {
        ...
        runSqlTest("features/jdbc/oracle-parallel/default");
        assertEquals(oracle.getIntValueFromQuery(
                String.format(GET_STATS_QUERY_TEMPLATE, "SELECT id, descr FROM " + dataSourcePath)), 0
        );
    }

    @Test(groups = {"features", "jdbc"}, description = "Set disable parallel for query")
    public void checkDisableQueryParallel() throws Exception {
        ...
        runSqlTest("features/jdbc/oracle-parallel/disable-query");
        assertEquals(oracle.getIntValueFromQuery(
                String.format(GET_STATS_QUERY_TEMPLATE, "SELECT id, descr FROM " + dataSourcePath)), 0
        );
    }

    @Test(groups = {"features", "jdbc"}, description = "Set 3 parallel sessions with force query")
    public void checkForceQueryWith3Parallel() throws Exception {
        ...
        runSqlTest("features/jdbc/oracle-parallel/force-query");
        assertEquals(oracle.getIntValueFromQuery(
