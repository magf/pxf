package org.greenplum.pxf.automation.features.fragment;
...
public class FragmentDistributionTest extends BaseFeature {
     ...
     */
    @Test(groups = {"features", "gpdb"}, description = "Check JDBC fragments distribution across segments with default policy (round-robin)")
    public void fragmentDistributionForJdbcWithDefaultPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/jdbc");
     ...
     */
    @Test(groups = {"features", "gpdb"}, description = "Check JDBC fragments distribution across segments. Run on limit segments")
    public void fragmentDistributionForJdbcWithLimitTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/jdbc-limit");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
     ...
     * and they will be split between 2 segment hosts evenly.
     */
    @Test(groups = {"features", "gpdb"}, description = "Check JDBC fragments distribution across segments with active-segment distribution policy")
    public void fragmentDistributionForJdbcWithActiveSegmentPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/jdbc-as-policy");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
            String checkPolicyResult = getCmdResult(cluster, String.format(PXF_LOG_CHECK_POLICY_TEMPLATE, "active-segment"));
     ...
     * The query will fail.
     */
    @Test(groups = {"features", "gpdb"}, description = "Check JDBC fragments distribution across segments with failed active-segment distribution policy")
    public void fragmentDistributionForJdbcWithFailedActiveSegmentPolicyTest() throws Exception {
        runSqlTest("features/fragment/fragment-distribution/jdbc-failed-as-policy");
    }
     ...
     * fragmentsCount has to be 8 as a summary from all segments.
     */
    @Test(groups = {"features", "gpdb"}, description = "Check JDBC fragments distribution across segments with random distribution policy")
    public void fragmentDistributionForJdbcWithRandomPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/jdbc-random-policy");
        int fragmentsCount = 0;
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
     ...
     */
    @Test(groups = {"features", "gpdb"}, description = "Check JDBC fragments distribution across segments with improved-round-robin distribution policy")
    public void fragmentDistributionForJdbcWithImprovedRoundRobinPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/jdbc-irr-policy");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
     ...
     */
    @Test(groups = {"features", "gpdb"}, description = "Check HDFS fragments distribution across segments. Run on all segments")
    public void fragmentDistributionForHdfsTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs");
        int fragmentsCount = 0;
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
     ...
     * and they will be split between 2 segment hosts evenly.
     */
    @Test(groups = {"features", "gpdb"}, description = "Check HDFS fragments distribution across segments. Run on 2 segments")
    public void fragmentDistributionForHdfsWithLimitTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-limit");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
            String checkPolicyResult = getCmdResult(cluster, String.format(PXF_LOG_CHECK_POLICY_TEMPLATE, "active-segment"));
     ...
     */
    @Test(groups = {"features", "gpdb"}, description = "Check HDFS fragments distribution across segments with 4 active-segment distribution policy")
    public void fragmentDistributionForHdfsWith4ActiveSegmentPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-4-as-policy");
        int fragmentsCount = 0;
     ...
     * and they will be split between 2 segment hosts evenly.
     */
    @Test(groups = {"features", "gpdb"}, description = "Check HDFS fragments distribution across segments with 2 active-segment distribution policy")
    public void fragmentDistributionForHdfsWith2ActiveSegmentPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-2-as-policy");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
            String checkPolicyResult = getCmdResult(cluster, String.format(PXF_LOG_CHECK_POLICY_TEMPLATE, "active-segment"));
     ...
     * fragmentsCount has to be 8 as a summary from all segments.
     */
    @Test(groups = {"features", "gpdb"}, description = "Check HDFS fragments distribution across segments with random distribution policy")
    public void fragmentDistributionForHdfsWithRandomPolicyTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-random-policy");
        int fragmentCount = 0;
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
     ...
     * distribute 6 fragments between all segments and the rest 2 fragments will be distributed between segment hosts evenly
     */
    @Test(groups = {"features", "gpdb"}, description = "Check HDFS fragments distribution across segments with improved-round-robin " +
    ...
    public void fragmentDistributionForHdfsWithIrrPolicyWithMoreFragmentsTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-irr-more-policy");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
            String checkPolicyResult = getCmdResult(cluster, String.format(PXF_LOG_CHECK_POLICY_TEMPLATE, "improved-round-robin"));
    ...
    public void fragmentDistributionForHdfsWithIrrPolicyWithEvenFragmentsTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-irr-even-policy");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
            String checkPolicyResult = getCmdResult(cluster, String.format(PXF_LOG_CHECK_POLICY_TEMPLATE, "improved-round-robin"));
    ...
    public void fragmentDistributionForHdfsWithIrrPolicyWithLessFragmentsTest() throws Exception {
        cleanLogs();
        runSqlTest("features/fragment/fragment-distribution/hdfs-irr-less-policy");
        for (Node pxfNode : pxfNodes) {
            cluster.copyFromRemoteMachine(pxfNode.getUserName(), pxfNode.getPassword(), pxfNode.getHost(), pxfLogFile, PXF_TEMP_LOG_PATH);
            copyLogs(getMethodName(), pxfNode.getHost());
            String checkPolicyResult = getCmdResult(cluster, String.format(PXF_LOG_CHECK_POLICY_TEMPLATE, "improved-round-robin"));
