package org.greenplum.pxf.automation.features.reload;
...
public class PxfReloadTest extends BaseFeature {
    ...
    private static final String GREP_SHUTDOWN_COMMAND = "cat " + PXF_TEMP_LOG_FILE + " | grep 'Shutdown completed.' | wc -l";
    private static final String GREP_RELOAD_COMMAND_TEMPLATE = "cat " + PXF_TEMP_LOG_FILE + " | grep 'Received a request to reload a profile' | grep \"%s\" | wc -l";
    ...
    public void reloadAllDuringRead() throws Exception {
        ...
        for (Node pxfNode : pxfNodes) {
            ...
            String result = getCmdResult(cluster, String.format(GREP_RELOAD_COMMAND_TEMPLATE, "profile=, server="));
            assertEquals("1", result);
            // Must be 2 records for all segments because there are only 2 queries.
            shutDownPoolCount += Integer.parseInt(getCmdResult(cluster, GREP_SHUTDOWN_COMMAND));
    ...
    public void reloadAllServersForProfileDuringRead() throws Exception {
        ...
        for (Node pxfNode : pxfNodes) {
            ...
            copyLogs(getMethodName(), pxfNode.getHost());
            // Must be 1 record on each segment host
            String result = getCmdResult(cluster, String.format(GREP_RELOAD_COMMAND_TEMPLATE, "profile=jdbc, server="));
            assertEquals("1", result);
            // Must be 2 records for all segments because there are only 2 queries.
            shutDownPoolCount += Integer.parseInt(getCmdResult(cluster, GREP_SHUTDOWN_COMMAND));
            cluster.deleteFileFromNodes(PXF_TEMP_LOG_FILE, false);
    ...
    public void reloadJdbcProfileDuringHdfsRead() throws Exception {
        ...
        for (Node pxfNode : pxfNodes) {
            ...
            copyLogs(getMethodName(), pxfNode.getHost());
            // Must be 1 record on each segment host
            String result = getCmdResult(cluster, String.format(GREP_RELOAD_COMMAND_TEMPLATE, "profile=jdbc, server=default"));
            assertEquals("1", result);
    ...
    public void reloadOneServerProfileDuringRead() throws Exception {
        ...
        for (Node pxfNode : pxfNodes) {
            ...
            copyLogs(getMethodName(), pxfNode.getHost());
            // Must be 1 record on each segment host
            String result = getCmdResult(cluster, String.format(GREP_RELOAD_COMMAND_TEMPLATE, "profile=jdbc, server=reload"));
            assertEquals("1", result);
